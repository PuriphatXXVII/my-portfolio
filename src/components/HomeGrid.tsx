    ...
    <div className="max-w-350 w-full mx-auto grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8 relative z-10">
      ...
      >
        ...
        <motion.img src={myPhoto} alt="Puriphat"
          className="absolute top-1/2 -right-12 md:-right-16 w-48 h-48 md:w-72 md:h-72 rounded-full object-cover object-[center_20%] border-4 border-white/40 shadow-2xl z-10"
          style={{ y: '-50%' }}
          animate={{ translateY: [0, -10, 0] }}
          transition={{ duration: 5, repeat: Infinity, ease: 'easeInOut' }} />

        {/* Rotating dashed ring */}
        <motion.div aria-hidden className="absolute top-1/2 -right-20 md:-right-24 w-64 h-64 md:w-96 md:h-96 -translate-y-1/2 rounded-full border-2 border-dashed border-white/20 pointer-events-none"
          animate={{ rotate: 360 }}
          transition={{ duration: 30, repeat: Infinity, ease: 'linear' }} />

        {/* Counter-rotating second ring */}
        <motion.div aria-hidden className="absolute top-1/2 -right-16 md:-right-20 w-56 h-56 md:w-80 md:h-80 -translate-y-1/2 rounded-full border border-white/10 pointer-events-none"
          animate={{ rotate: -360 }}
